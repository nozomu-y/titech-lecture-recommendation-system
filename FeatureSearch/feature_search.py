...
class FeatureSearch:
    ...
    def set_major(self, course_num):
        if (course_num < -1 or len(self.course) < course_num):
            print("Unexpected value input.")
            sys.exit(1)
        self.course_num = course_num

    def set_day(self, day):
        if len(day) != len(self.day):
            print("Unexpected value input.")
            sys.exit(1)
        for i in range(len(self.day)):
            if day[i] != 0 and day[i] != 1:
                print("Unexpected value input.")
                sys.exit(1)
        self.day_select = day

    def set_period(self, period):
        if len(period) != len(self.period):
            print("Unexpected value input.")
            sys.exit(1)
        for i in range(len(self.period)):
            if period[i] != 0 or period[i] != 1:
                print("Unexpected value input.")
                sys.exit(1)
        self.period_select = period

    def set_quarter(self, quarter):
        if len(quarter) != len(self.quarter):
            print("Unexpected value input.")
            sys.exit(1)
        for i in range(len(self.quarter)):
            if quarter[i] != 0 or quarter[i] != 1:
                print("Unexpected value input.")
                sys.exit(1)
        self.quarter_select = quarter

    def set_textbook(self, textbook):
        if textbook != 0 and textbook != 1 and textbook != -1:
            print("Unexpected value input.")
            sys.exit(1)
        self.is_need_textbook = textbook

    def set_exam(self, exam):
        if exam != 0 and exam != 1 and exam != -1:
            print("Unexpected value input.")
            sys.exit(1)
        self.is_need_assessment[0] = exam

    def set_report(self, report):
        if report != 0 and report != 1 and report != -1:
            print("Unexpected value input.")
            sys.exit(1)
        self.is_need_assessment[1] = report

    def set_presentation(self, presentation):
        if presentation != 0 and presentation != 1 and presentation != -1:
            print("Unexpected value input.")
            sys.exit(1)
        self.is_need_assessment[2] = presentation
